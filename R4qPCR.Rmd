...
- *export* to *RDML* format file via *Bio-Rad CFX Manager 3.1* software
- *RDML* file *import* into **R** by **RDML** package and experiment overview
- *melting curves analysis* to test reactions specificity by **MBmca** package
- *quantification cycle (Cq) calculation* by **qPCR** package
- genes *amplification efficiency* determination by **chipPCR** package
- *gene expression analysis* by **EasyqpcR** package


## Export to RDML Format

Default format of *Bio-Rad CFX Manager 3.1* files is *pcrd* that is password protected zip file which can't be imported directly into **R**. So we need to convert it to *RDML* inside *Bio-Rad CFX Manager 3.1*.
...
rdml$GetFData(dp.type = "mdp", # get melting data
  ...
  mutate(dfluor = tryCatch(c(NA,
                             ...
                             diffQ(data.frame(tmp, fluor), verbose = TRUE, fct = max)$xy[, 2],
                             NA),
                           error = function(e) NA)) %>% 
  ggplot() +
...
To calculate gene expression we will use *∆∆Ct* method. *Actin* and *Tubulin* will be our reference genes. *0Hr* — control sample.
